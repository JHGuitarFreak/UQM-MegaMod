...
_GetFontData (uio_Stream *fp, DWORD length)
{
	...
	canvas = TFB_DrawCanvas_LoadFromFile (fontDirHandle, "leading.png");

	if (canvas == NULL)
	{
		char *basename = strrchr (_cur_resfile_name, '/') + 1;
		log_add (log_Debug,
				"%s does not contain a \"leading.png\" file...\n"
				"Leading will be generated by %s height.", 
				basename, basename
			);
	}
	else
		TFB_DrawCanvas_GetExtent (canvas, &Leading);
	...
	if (Leading.height == 0)
		...
		fontPtr->Leading = Leading.height;

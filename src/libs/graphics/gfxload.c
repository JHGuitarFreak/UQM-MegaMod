...
_GetFontData (uio_Stream *fp, DWORD length)
{
	...
	const char *extra_data = "data.cfg";
	...
	for (dirEntryI = 0; dirEntryI < numDirEntries; dirEntryI++)
	{
		...
		if (strcmp (char_name, extra_data) == 0)
			continue;
	...
	if (_cur_resfile_name != 0)
	{
		const char *basename = strrchr (_cur_resfile_name, '/') + 1;

		if (fileExists2 (fontDirHandle, extra_data))
		{
			uio_Stream *cfgFile =
					uio_fopen (fontDirHandle, extra_data, "r");

			if (cfgFile)
			{
				char *CurrentLine[PATH_MAX];

				uio_fgets (CurrentLine, sizeof (CurrentLine), cfgFile);
				if (sscanf (CurrentLine, "%s %d", filename, &Leading) != 2)
				{
					log_add (log_Debug,
								"Error reading %s data from %s...\n"
								"Leading will be generated by %s height.",
								basename, extra_data, basename
					);
				}
			}
			else
	...
	if (Leading == 0)
		fontPtr->Leading += RES_SCALE (1); // But why?
	else
	{

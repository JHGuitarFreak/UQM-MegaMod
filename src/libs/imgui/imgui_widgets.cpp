...
static bool SliderBehaviorRangeT (const ImRect &bb, ImGuiID id, ImGuiDataType data_type, TYPE *v_min_val, TYPE *v_max_val, TYPE v_min, TYPE v_max, const char *format, const char *format_max, ImGuiSliderFlags flags, ImRect *out_grab_bb_min, ImRect *out_grab_bb_max, int *p_handle, TYPE step)
{
    ...
    auto grab_pos_from_value = [&](TYPE v) {
        float t = ImGui::ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE> (data_type, v, v_min, v_max, logarithmic_zero_epsilon, zero_deadzone_halfsize);
    ...
    if (g.ActiveId == id)
    {
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            else
            {
                ...
                if (*p_handle == -1)
                {
                    ...
                    TYPE new_v_min = ImGui::ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE> (data_type, t_min_new, v_min, v_max, logarithmic_zero_epsilon, zero_deadzone_halfsize);
                    TYPE new_v_max = ImGui::ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE> (data_type, t_max_new, v_min, v_max, logarithmic_zero_epsilon, zero_deadzone_halfsize);
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Keyboard || g.ActiveIdSource == ImGuiInputSource_Gamepad)
        {
            ...
            else if (g.SliderCurrentAccumDirty)
            {
                ...
                clicked_t = ImGui::ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE> (data_type, *active_val, v_min, v_max, logarithmic_zero_epsilon, zero_deadzone_halfsize);
                ...
                else
                {
                    ...
                    float old_clicked_t = clicked_t;
                    clicked_t = ImSaturate (clicked_t + delta);
                    TYPE v_new = ImGui::ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE> (data_type, clicked_t, v_min, v_max, logarithmic_zero_epsilon, zero_deadzone_halfsize);
                    if (is_floating_point && !(flags & ImGuiSliderFlags_NoRoundToFormat))
                        v_new = ImGui::RoundScalarWithFormatT<TYPE> ((*p_handle == 0) ? format : format_max, data_type, v_new);
                    float new_clicked_t = ImGui::ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE> (data_type, v_new, v_min, v_max, logarithmic_zero_epsilon, zero_deadzone_halfsize);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (is_visible)
    {
        ...
        if (highlighted || selected)
        {
            ...
            RenderFrame(bb.Min, bb.Max, col, true, style.SelectablesRounding);
        }
        if (g.NavId == id)
        {

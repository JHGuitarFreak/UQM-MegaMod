...
#ifdef HAVE_JOYSTICK
			result = VControl_AddControllerAxisBinding (g->gesture.controller_axis.port,
				g->gesture.controller_axis.axis,
				(g->gesture.controller_axis.polarity < 0) ? -1 : 1, target);
			break;
#endif /* HAVE_JOYSTICK */
		case VCONTROL_CONTROLLERBUTTON:
#ifdef HAVE_JOYSTICK
			result = VControl_AddControllerButtonBinding (g->gesture.controller_button.port,
...
VControl_RemoveGestureBinding (VCONTROL_GESTURE *g, int *target)
{
	switch (g->type)
	{
		...
		case VCONTROL_CONTROLLERAXIS:
#ifdef HAVE_JOYSTICK
			VControl_RemoveControllerAxisBinding (g->gesture.controller_axis.port,
				g->gesture.controller_axis.axis,
				(g->gesture.controller_axis.polarity < 0) ? -1 : 1, target);
			break;
#endif /* HAVE_JOYSTICK */
		case VCONTROL_CONTROLLERBUTTON:
#ifdef HAVE_JOYSTICK
			VControl_RemoveControllerButtonBinding (g->gesture.controller_button.port,
				g->gesture.controller_button.button, target);
			break;
#endif /* HAVE_JOYSTICK */
		case VCONTROL_NONE:

...
START_GAME_STATE
	...
	ADD_GAME_STATE (MELNORME_TECH_STACK, 4) // Serosis look into
 ...
 * shouldn't be part of the serialized Game State array! --MCM */
	...
	ADD_GAME_STATE (HM_ENCOUNTERS, 4)
...
#define MET_AN_UTWIG (GET_GAME_STATE (UTWIG_HAVE_ULTRON) \
		|| GET_GAME_STATE (UTWIG_WAR_NEWS))

#define MET_A_DRUUGE ((!GET_GAME_STATE (DRUUGE_MANNER) \
		&& GET_GAME_STATE (DRUUGE_VISITS)) \
		|| GET_GAME_STATE (DRUUGE_HOME_VISITS) \
		|| GET_GAME_STATE (DRUUGE_HOME_INFO) \
		|| GET_GAME_STATE (DRUUGE_SPACE_INFO) \
		|| GET_GAME_STATE (KNOW_DRUUGE_SLAVERS))

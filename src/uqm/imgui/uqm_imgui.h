...
BOOLEAN UQM_ImGui_CheckBox (const char *label, OPT_ENABLABLE *v,
		const char *key, bool needs_reboot);
bool ImGui_SizedComboChar (const char *label, int *curr_item,
		const char *const items[], int items_count);
BOOLEAN UQM_ComboChar (const char *label, const char *const items[],
		int items_count, int *option, const char *key, bool reload);
BOOLEAN UQM_ConsComboChar (const char *label, const char *const items[],
